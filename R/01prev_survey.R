...
library(here)                           #file paths etc
library(lattice)                        #plots


## Data inputs loaded here
load(here('Rdata/prev.Rdata'))          #prevalence survey data
load(here("Rdata/pop.Rdata"))           #population data
load(here("Rdata/tb.Rdata"))            #notification data
...
ggplot(data=prev[sex=='a' & age.group %ni% c('10plus','15plus','5plus','50plus',
                                             '3monthplus'),],
...
prev <- prev[!(iso3=='PHL' & !year==(estyr-1))]
...
makeregionplot <- function(xtradata,reg='AFR',alph=.1){
 PP <-  ggplot(data=PR[g.whoregion==reg],
     ...
     stat_summary(data=xtradata,geom="line", fun.y=median)+
...
MFD <- MFD[iso3.year %ni% c('CHN 1990','CHN 2000',
...
SampleRRs <- function(n=1,iso3,verbose=TRUE){
  ...
  if(is.na(srcinc) | srcinc != 'Standard adjustment' & reg!='EUR'){ #o/w use notifications
...
getProps <- function(n=1,cn,verbose=TRUE,shape=FALSE){
  ...
  if('P' %in% names(smps)){
    ...
    if(shape){   #convert from list to data.table
      ...
      ans <- cbind(rep=rep(1:(nrow(ans)/2),each=2),sex=rep(1:2,(nrow(ans)/2)),ans)
      ans <- as.data.frame(ans)
      ans <- data.table::melt(ans,id=c('rep','sex'))
...
##
prevsplits <- prevsplitsL <- prevsrc <- list()
for(cn in unique(est$iso3)){
  if(nrow(est[iso3==cn & year==estyr,])>0){ #skip things without estimates
    txt <- capture.output( Pdt <- getProps(n=1e2,cn=cn,verbose = TRUE,shape=TRUE) )
...
## save out results:
save(file=here('prevsplits/prevsplits.Rdata'),prevsplits)
save(file=here('prevsplits/prevsplitsL.Rdata'),prevsplitsL)
save(file=here('prevsplits/prevsrc.Rdata'),prevsrc)
...
for(cn in unique(prevsplits$iso3)){
  if(!is.na(prevsplits[iso3==cn,sum(prop)])){
    plt <- countryplot(cn)
    suppressMessages(ggsave(here("prevsplits/plots",paste0(cn,".pdf"))))
  }
}
