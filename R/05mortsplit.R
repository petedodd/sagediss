...
load(here('Rdata/pop.Rdata'))         # populations for country name
load(here('Rdata/unaids.Rdata'))          # includes notifications
load(here('R/02children/mortoutput/KM.Rdata'))   # Child CFR estimates
load(here('incsplits/data/incsplit.Rdata')) #from prevalence split work


## If Philippe's `regional.Rdata` file is ready, use it; otherwise, make one.
fn <- here('Rdata/regional.Rdata')
if(!file.exists(fn)){
  ...
  regional <- est[,{
    ...
    ## mort HIV-ve
    mort.nh.num=smn(mort.nh.num); ss = sqrt(smn((mort.nh.hi.num-mort.nh.lo.num)^2))/3.92;
    mort.nh.lo.num=mort.nh.num - 1.96*ss; mort.nh.hi.num=mort.nh.num + 1.96*ss;
    ## mort HIV+ve
    mort.h.num=smn(mort.h.num);
    ss = sqrt(smn((mort.h.hi.num-mort.h.lo.num)^2))/3.92;
    mort.h.lo.num=mort.h.num - 1.96*ss; mort.h.hi.num=mort.h.num + 1.96*ss;
...
if(!file.exists(fn)){
  ...
  global <- est[,{
    ...
    mort.nh.lo.num=mort.nh.num - 1.96*ss; mort.nh.hi.num=mort.nh.num + 1.96*ss;
    ## mort HIV+ve
    mort.h.num=smn(mort.h.num); ss = sqrt(smn((mort.h.hi.num-mort.h.lo.num)^2))/3.92;
    mort.h.lo.num=mort.h.num - 1.96*ss; mort.h.hi.num=mort.h.num + 1.96*ss;
    list(inc.num=inc.num,inc.lo.num=inc.lo.num,inc.hi.num=inc.hi.num,
         mort.nh.num=mort.nh.num,mort.nh.lo.num=mort.nh.lo.num,
         mort.nh.hi.num=mort.nh.hi.num,
...
xlfn <- here('Rdata/VR/info_TB_may19.xls')

## big loop to generate cleaned/reshaped file if not there:
mfn <- here('mortsplits/VR.Rdata')
if(!file.exists(mfn)){
  ## reading in excel data

  ## -------- ICD>10.1
  M1 <- as.data.table(read_excel(xlfn,sheet = 4,skip=1,col_types=typz)) #ICD > 10.1
  M1 <- refrm(M1)
  ## -------- ICD 9
  M2 <- as.data.table(read_excel(xlfn,sheet = 5,skip=1,col_types=typz)) #ICD = 9
  M2 <- refrm(M2)
  ## -------- ICD 8
  M3 <- as.data.table(read_excel(xlfn,sheet = 6,skip=1,col_types=typz)) #ICD = 8
  M3 <- refrm(M3)
  ## -------- ICD 10.1
  M4 <- as.data.table(read_excel(xlfn,sheet = 7,skip=1,col_types=typz)) #ICD = 10.1
  M4 <- refrm(M4)
  ## SA TB/HIV mortality data?
  ...
  ## sub-countries
  VR[name %in% c("French Guiana","Martinique","Reunion","Mayotte"),name:="France"]
  ...
  ## ## VR from IHME
  ## ihme <- scan(here('mortsplits/vr_ihme_iso3.txt'),'character')
  ## IHME <- fread(here('input/IHME/IHME_GBD_2013_DEATHS_1990_2013_TUBERCULOSIS_Y2014M12D17.CSV'))
  ## IHME <- IHME[unit=='number']          #restrict
  ## ## join
  ## ihmedat <- data.table(iso3=ihme)
  ## ihmedat <- merge(ihmedat,unique(pop[,.(iso3,country)]),by='iso3')
  ## ihmedat$country[!(ihmedat$country %in% unique(IHME$location_name))]#OK to join byname?
  ## (ihmenm <- grep('Bol',unique(IHME$location_name),value=TRUE))
  ## IHME[location_name==ihmenm,location_name:=ihmedat[iso3=='BOL',country]] #rename
  ## IHME <- IHME[location_name %in% ihmedat$country]     #restrict to countries wanted
  ## IHME <- merge(IHME,ihmedat,by.x='location_name',by.y='country',all.x = TRUE)
  ## IHME <- IHME[,.(iso3,name=location_name,year,age_group_name,sex_name,mean)]
  ## IHME <- IHME[,.SD[year==max(year)],by=iso3] #most recent year
  ##   ## recoding
  ## IHME[,sex:='M']; IHME[sex_name=='Female',sex:='F']; IHME[,sex_name:=NULL] #sex
  ## IHME <- IHME[age_group_name != "All Ages"]
  ## IHME[,acat:='65plus']                 #default most complex
  ## IHME[age_group_name %in% c("Post Neonatal","1 to 4"),acat:='0_4']
  ## IHME[age_group_name %in% c("5 to 9","10 to 14"),acat:='5_14']
  ## IHME[age_group_name %in% c("15 to 19","20 to 24"),acat:='15_24']
  ## IHME[age_group_name %in% c("25 to 29","30 to 34"),acat:='25_34']
  ## IHME[age_group_name %in% c("35 to 39","40 to 44"),acat:='35_44']
  ## IHME[age_group_name %in% c("45 to 49","50 to 54"),acat:='45_54']
  ## IHME[age_group_name %in% c("55 to 59","60 to 64"),acat:='55_64']
  ## IHME <- IHME[,.(value=sum(mean)),by=.(iso3,name,sex,age_group=acat)]
  ## setdiff(ihme,IHME$iso3)               #OK
  ## IHME[,c('ill','tot','g'):=NA]

  ## ## are these in VR?
  ## ihme[which(ihme %in% VR$iso3)]
  ## VR[iso3=='ZAF',sum(value)]
  ## ggplot(VR[iso3=='ZAF'],aes(x=age_group,y=value,col=sex)) +
  ##   geom_point() +
...
getVRMortnh <- function(cn,KMoverride=TRUE,allplt=FALSE){
  ...
  mfy <- tmpi[age_group=='0_4' & sex=='M',inc]/tmpi[age_group=='0_4',sum(inc)+1e-15]
  mfo <- tmpi[age_group=='5_14' & sex=='M',inc]/tmpi[age_group=='5_14',sum(inc)+1e-15]
  tmp[age_group=='0_4' & sex=='M',mort := mfy * km[,deathsY*(1-h)]]   # young,m
  tmp[age_group=='0_4' & sex=='F',mort := (1-mfy) * km[,deathsY*(1-h)]]   # young,f
  tmp[age_group=='5_14' & sex=='M',mort := mfo * km[,(deaths-deathsY)*(1-h)]]  # old,m
  tmp[age_group=='5_14' & sex=='F',mort := (1-mfo) * km[,(deaths-deathsY)*(1-h)]] # old,f
  ## adults
...
##
## **NB** assumption of no age-dependence in CFR. 
## 
...
## young people 15-24
HYP <- fread(here('mortsplits/hivinput/hivYP.csv'),header=TRUE)
HYP <- HYP[,.(Country,`2018`)]
HYP <- HYP[,hivyp:=as.numeric(gsub("[[:space:]+|<+|\\.+]","",`2018`))]
## older folk 50+
HO <- fread(here('mortsplits/hivinput/hivOLD.csv'),header=TRUE)
HO <- HO[,.(Country,`2018`)]
HO <- HO[,hivo:=as.numeric(gsub("[[:space:]+|<+|\\.+]","",`2018`))]
HO[,`2018`:=NULL]; HYP[,`2018`:=NULL]
...
##
(bad <- setdiff(HA[,unique(Country)],est[,unique(country)]))


HA <- merge(HA,est[year==estyr][,.(iso3,country)],by.x='Country',by.y='country',all.x = TRUE)
## drop NAs
...
getCFRMortH <- function(cn,allplt=FALSE){
  ...
  tmp[age_group=='0_4' & sex=='M',mort := mfy * km$deathsY ]                  #young,m
  tmp[age_group=='0_4' & sex=='F',mort := (1-mfy) * km$deathsY ]              #young,f
  tmp[age_group=='5_14' & sex=='M',mort := mfo * km[,deaths-deathsY] ]          #old,m
  tmp[age_group=='5_14' & sex=='F',mort := (1-mfo) * km[,deaths-deathsY] ]      #old,f
  ## adults
  ...
  CFR.nh <- (el$mort.nh.num - km$deaths + km$deathsH) / (ainc*(1-h)+1e-15)
  ...
  hs <- getHIVsplit(cn)
  pa <- hs$pa; sr <- hs$sr;
  hm <- h * sr * pa; hf <- h * (1-sr) * pa; #HIV split among M/F TB cases
  ...
  tmph <- copy(tmp); tmpnh <- copy(tmp)        #separate copies for HIV+/- below
  tmp <- tmp[,.(iso3,sex,age_group,mort,src.mort,CFR)]
  ## --- HIV + / - splits separately (for tables -- see below)
  ## -- HIV +ve
  tmph[,mort:=0.0]
  tmph[age_group=='0_4' & sex=='M',mort := mfy * km[,deathsY*h] ]   #young,m
  tmph[age_group=='0_4' & sex=='F',mort := (1-mfy) * km[,deathsY*h] ]     #young,f
  tmph[age_group=='5_14' & sex=='M',mort := mfo * km[,(deaths-deathsY)*h] ] #old,m
  tmph[age_group=='5_14' & sex=='F',mort := (1-mfo) * km[,(deaths-deathsY)*h] ]#old,f
  ...
  ## -- HIV -ve
  tmpnh[,mort:=0.0]
  tmpnh[age_group=='0_4' & sex=='M',mort := mfy * km[,deathsY*(1-h)] ]   #young,m
  tmpnh[age_group=='0_4' & sex=='F',mort := (1-mfy) * km[,deathsY*(1-h)] ]     #young,f
  tmpnh[age_group=='5_14' & sex=='M',mort := mfo * km[,(deaths-deathsY)*(1-h)] ] #old,m
  tmpnh[age_group=='5_14' & sex=='F',mort := (1-mfo)*km[,(deaths-deathsY)*(1-h)] ]#old,f
  ...
  mpnh <- mph <- NA
  if(allplt){
    mpnh <- muplot(tmpnh,el[,country])
    mph <- muplot(tmph,el[,country])
  }
  list(dat=tmp,plt=mp,pltnh=mpnh,plth=mph,
...
## save(allcns,file=here('mortsplits/allcns.Rdata'))
save(allcnsH,file=here('mortsplits/allcnsH.Rdata'))
save(allcnsNH,file=here('mortsplits/allcnsNH.Rdata'))
save(HNsplit,file=here('mortsplits/HNsplitL.Rdata'))
save(HPsplit,file=here('mortsplits/HPsplitL.Rdata'))

## Country plots

## HIV-ve
fn <- glue(here('mortsplits/plots/nhMort_'))
j <- k <- 0
...
## HIV+ve
fn <- glue(here('mortsplits/plots/hMort_'))
j <- k <- 0
...
HNsplit <- merge(HNsplit,est[year==estyr][,.(iso3,g.whoregion)],by='iso3',all.y = FALSE)
save(HNsplit,file=here('mortsplits/HNsplit.Rdata'))


HPsplit <- do.call('rbind',HPsplit)
HPsplit <- merge(HPsplit,est[year==estyr][,.(iso3,g.whoregion)],by='iso3',all.y = FALSE)
save(HPsplit,file=here('mortsplits/HPsplit.Rdata'))
...
##
load(here('mortsplits/HNsplit.Rdata')) #to be overwritten
HNsplit <- merge(HNsplit,est[year==estyr][,.(iso3,mort.nh.num)],by='iso3')
HNR <- HNsplit[,.(mort=weighted.mean(mort,w=mort.nh.num,na.rm=TRUE)),
               by=.(sex,age_group,g.whoregion)] #regional averages
extras <- est[year==estyr][iso3 %ni%HNsplit[,iso3],.(iso3,g.whoregion,mort.nh.num)]
...
HNsplit <- rbind(HNsplit[,.(iso3,sex,age_group,mort,src.mort,g.whoregion)],extras)
...
## uncertainty
test <- HNsplit[,.(tt=(hi-lo)^2,tt2=(mort.nh.hi.num-mort.nh.lo.num)^2),by=iso3]
test[,summary(tt)]
...
fn <- here('mortsplits/db_mortality_disaggregated_') + getdate() + '.csv'
fwrite(HNsplit,file=fn)
