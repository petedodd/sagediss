## ======== mortality estimates ===========
## adapted code from 4PM repo and 2017 LGH paper
...
load(here('incsplits/data/incsplit.Rdata'))
...
buryo <- incsplit[BB[,as.character(iso3)]][age_group=='0_4',.(inc=sum(inc)),by=iso3]
burol <- incsplit[BB[,as.character(iso3)]][age_group=='5_14',.(inc=sum(inc)),by=iso3]
...
for(i in 1:nrep){
    ...
    DeathsOffTxY <- pmax(buryo$inc-F*BB$notifsY,0) * cfrz0y * (1-tbhiv)
    DeathsOnTxHY <- F*BB$notifsY * (cfrzh1aY * tbart + cfrzh1Y * (1-tbart)) * tbhiv 
    DeathsOffTxHY <- pmax(buryo$inc-F*BB$notifsY,0) * tbhiv * (cfrzh0aY * tbart + cfrzh0Y * (1-tbart)) #averages CFR
    ## --old
    DeathsOnTxO <- F*BB$notifsO * cfrz1o * (1-tbhiv)
    DeathsOffTxO <- pmax(burol$inc-F*BB$notifsO,0) * cfrz0o * (1-tbhiv)
    DeathsOnTxHO <- F*BB$notifsO * (cfrzh1aO * tbart + cfrzh1O * (1-tbart)) * tbhiv
    DeathsOffTxHO <- pmax(burol$inc-F*BB$notifsO,0) * tbhiv * (cfrzh0aO * tbart + cfrzh0O * (1-tbart)) #averages CFR
    ## record results
...
RESA$g_whoregion<- BB$g.whoregion
...
if(!sensNote & !useHIVneg){
    png(here('R/02children/plots/global_hist.png'))
...
gl <- c(unlist(RESATs[,5:8,with=FALSE]),total=quantile(RESAGnca$total,.025))
gh <- c(unlist(RESATs[,9:12,with=FALSE]),total=quantile(RESAGnca$total,.975))
...
fn <- here('R/02children/mortoutput',fn)
cat(pcuntx,file=fn)
...
RESAC <- RESA[,.(deaths = midr(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH),
                 ...
                 ## old no HIV+/-
                 deathsHO = midr(DeathsOnTxH+DeathsOffTxH-DeathsOnTxHY+DeathsOffTxHY),
                 deathsHOlo = lbr(DeathsOnTxH+DeathsOffTxH-DeathsOnTxHY-DeathsOffTxHY),
                 deathsHOhi = ubr(DeathsOnTxH+DeathsOffTxH-DeathsOnTxHY-DeathsOffTxHY),
                 deathsNHO = midr(DeathsOnTx+DeathsOffTx-DeathsOnTxY+DeathsOffTxY),
                 deathsNHOlo = lbr(DeathsOnTx+DeathsOffTx-DeathsOnTxY-DeathsOffTxY),
                 deathsNHOhi = ubr(DeathsOnTx+DeathsOffTx-DeathsOnTxY-DeathsOffTxY),
                 ## previous stuff
...
KM <- RESAC
save(KM,file=here('R/02children/mortoutput/KM.Rdata'))
