...
## some requisite data-------------------------------------------
load(here('R/02children/data/LAT.Rdata'))             #latitude
load(here('Rdata/pop.Rdata'))                                          #demography
load(here('Rdata/unaids.Rdata'))                                       #HIV prev

## update here
estyr <- 2018                              # pdate
load(here('R/02children/data/eprev17.Rdata'))      #TB prevalence old
load(here('R/02children/data/est17.Rdata'))        #TB est old
estold <- copy(est)
...
BCGfn <- here('R/02children/data/BCG.Rdata')
if(!file.exists(BCGfn)){
  BCG <- readxl::read_xls(here('R/02children/data',
                               'country_coverage.xls'),
                          sheet="BCG")
  BCG1 <- as.data.table(BCG)[,.(iso3=ISO_code,bcg=`2017`)]
  BCG <- readxl::read_xls(here('R/02children/data',
                               'WUENIC.xls'),
                          sheet="world_coverage")
  BCG2 <- as.data.table(BCG)[Vaccine=='BCG',.(iso3=ISO_code,bcg=Percent_covrage,Year)]
  BCG2[,table(Year)]
  mis18 <- setdiff(BCG2[Year==2017,unique(iso3)],BCG2[Year==2018,unique(iso3)])
  mis18 %in% BCG1[,unique(iso3)]        #all in other
  cf <- merge(BCG1[,.(iso3,report=bcg)],BCG2[Year==2017][,.(iso3,WUENIC=bcg)],all=TRUE)
  load(here('Rdata/tb.Rdata'))                                          #g.whoregion
  whokey <- unique(tb[,.(iso3,g.whoregion)])
  cfm <- melt(cf,id='iso3')
  cfm <- merge(cfm,whokey)
  ## decision: use WUENIC with most recent year
  BCG <- BCG2[,.SD[Year==max(Year)],by=iso3]
  save(BCG,file=BCGfn)
} else{
  load(BCGfn)
}
...
summary(Dr$LAT)
Dr[is.na(LAT),.(iso3,country)]          #missing LAT
...
Dr[!is.finite(pop014),pop014:= as.integer(`<1` + `1` + `2-4` + `5-9` + `10-14`)]
...
metaparm <- list(drs1 = drs1,drs2 = drs2,drs1d = drs1d,drs2d = drs2d, #progression
...
parm <- list(prPTB=c(0.35,0.15,0.05,0.02,0.15), #risk of progression to PTB
             prDTB=c(0.15,0.035,0.005,0.005,0.005), #risk of progression TBM/miliary
...
makeParameters2 <- function(LH,mp){
    ...
    p1z <- qbeta(LH[,1:5],shape1=matrix(mp$drs1,ncol=5,nrow=nr,byrow=TRUE),
                 shape2=matrix(mp$drs2,ncol=5,nrow=nr,byrow=TRUE))
    p2z <- qbeta(LH[,1:5 + 5],shape1=matrix(mp$drs1d,ncol=5,nrow=nr,byrow=TRUE),
                 shape2=matrix(mp$drs2d,ncol=5,nrow=nr,byrow=TRUE))
    ## BCG
    vdz <- qbeta(LH[,1:5 + 10],shape1=matrix(mp$vds1,ncol=5,nrow=nr,byrow=TRUE),
                 shape2=matrix(mp$vds2,ncol=5,nrow=nr,byrow=TRUE))
    vpz <- qbeta(LH[,1:5 + 15],shape1=matrix(mp$vs1,ncol=5,nrow=nr,byrow=TRUE),
    ...
    ## betas
    bz <- qlnorm(LH[,22],meanlog=mp$bmlg,sd=mp$bsd)         #different LHS thing
    ## HIV and TB prevalence data
    tb <- qgamma(LH[,23:(22+NC)],shape = matrix(Dr$shpz,ncol=NC,nrow=nr,byrow=TRUE),
...
loopAns2 <- function(LP,mp,method=c('comm','lat')){
    ...
    ans <- data.frame(country=rep(Dr$country,n),a=0.0,b=0.0,s=0.0,d=0.0,f=0.0,
                      replicate=rpl, iso3=rep(Dr$iso3,n),
                      ARI=method[1],LAT=method[2])
    names(ans)[2:6] <- c("<1","1","2-4","5-9","10-14")
    ans <- as.data.table(ans)
    ans2 <- copy(ans)
    ans4 <- data.table(country=rep(Dr$country,n),ariz=0.0,replicate=rpl,ARI=method[1],LAT=method[2],iso3=rep(Dr$iso3,n)) #recording ARIs
    for(i in 1:n){
        ...
        hiv <- Dr$hivu15
        hiv[hiv>0] <- LP$h[i,]   # random sample for the countries with HIV
        ## make ARI
        ari <- LP$tb[i,] / 1e5
        vg <- 1
        if( method[1]=='comm'){
        ...
        tmp <- getIncidence(Dr,parms,ari,hiv,method=method[2]) #change here
...
make2cat <- function(L){#aggregate results
    ...
    ans2 <-  data.frame(country=rep(ans$country,2),iso3=rep(ans$iso3,2),value=0,age = c(rep('0-4',n),rep('5-14',n)),
