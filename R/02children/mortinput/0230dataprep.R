...
if(file.exists(here('R/02children/mortinput/BB.Rdata'))) stop('BB.Rdata already exists: not running 0230dataprep.R!')
print("BB.Rdata not detected: running paed mortality data preparation script.")
...
## ## compare (see end)
## load('BBold.Rdata')
## BBO <- as.data.table(BB)
estyr <- 2018                           #NOTE

## needs hi/lo from est
## --- data ----
load(here('Rdata/tb.Rdata'))          #notifications
load(here('R/02children/plots/K.Rdata'))
load(here('Rdata/est.Rdata')) #NOTE change
load(here('Rdata/pop.Rdata'))
load(here('Rdata/unaids.Rdata'))
load(here('R/02children/mortinput/AF.Rdata')) #age splits from previous paper
A <- fread(here('R/02children/mortinput/ARTpc_Population_Children.csv'),
           header=TRUE)

## --------work----------
B <- K[,.(iso3,mid=inc.num*propu15,var=(inc.num*propu15.sd)^2)] #bug here (nb inc var included in prop.sd)
B[,inc.num.014:=mid]
...
N[,sum(is.na(notifs))]                   #37 use most recent
...
## BB <- merge(BB,
##             est[year==estyr][,.(iso3,year,inc.num, #NOTE
##                                inc.num.lo=inc.lo*inc.num/inc,
##                                inc.num.hi=inc.hi*inc.num/inc)],
##             by='iso3')
BB <- merge(BB,
...
tmp <- BB[,.(tmp=mean(HIVmid/e.pop.014,na.rm=TRUE),
             tmp.se=mean((HIVhi-HIVlo)/(2*e.pop.014),na.rm=TRUE)),by=g.whoregion]
BB <- merge(BB,tmp,by='g.whoregion',all.x=TRUE,all.y=FALSE)
...
## inspect if reasonable representation of burden
who <- 200                                #choose test country
png(here('R/02children/mortoutput',paste0('test_HIV_',BB$iso3[who],'.png')))
...
png(here('R/02children/mortoutput',paste0('test_ART_',BB$iso3[who],'.png')))
...
## save/load here!
save(BB,file=here('R/02children/mortinput/BB.Rdata'))
