...
load(here('Rdata/tb.Rdata'))          # notifications
load(here('Rdata/pop.Rdata'))         # populations
load(here('R/02children/plots/K.Rdata')) #child estimates
load(here('incsplits/data/incsplit.Rdata')) #incidnce splits
...
TR <- regional[year==estyr][,.(g.whoregion,inc.num,inc.lo.num,inc.hi.num)]
...
db2 <- merge(incsplit[,.(iso3,sex=tolower(sex),age_group=age,prop)],
             est[year==estyr,.(iso3,year,inc.num,inc.lo.num,inc.hi.num,g.whoregion)],
             by='iso3',all.y = FALSE)
db2[,inc:=inc.num*prop]                     #incidence in this country/cat
db2[,prop:=inc/sum(inc)] #renormalize to whole world
...
dbf <- merge(dbf,regional[year==estyr,.(g.whoregion,inc.num,inc.lo.num,inc.hi.num)],
             by='g.whoregion',
             all.x=TRUE)
...
## to keep
db2o <- dbf[,.(group_type='g_whoregion',group_name=g.whoregion,year,measure,
...
tmp1 <- db2o[group_name!='global',.(suminc=sum(best)),by=.(age_group,sex)]
tmp2 <- db2o[group_name=='global',.(glob=sum(best)),by=.(age_group,sex)]
(tmp <- merge(tmp1,tmp2))

## write out
fn2 <- here('incsplits/data/db_estimates_group_') + getdate() + '.csv'
fwrite(db2o,file=fn2)
...
dbreg <- merge(db1f,est[year==estyr][,.(iso3,g.whoregion,inc.num)],by='iso3')
## dbreg <- dbreg[,.(prop=mean(prop)),by=.(g.whoregion,sex,age_group,pattern)] #reg averages
dbreg <- dbreg[,.(prop=weighted.mean(prop,inc.num)),by=.(g.whoregion,sex,age_group,pattern)] #reg averages
## countries with data
db1f <- merge(db1f,est[year==estyr][,.(iso3,year,inc.num,inc.lo.num,inc.hi.num)],
              by='iso3',all.x=TRUE)

## countries using regional averages
db10 <- merge(est[year==estyr][,.(iso3,year,inc.num,inc.lo.num,inc.hi.num,g.whoregion)],
              dbreg,
...
## write out
fn <- here('incsplits/data/db_estimates_country_') + getdate() + '.csv'
fwrite(db1f[order(iso3),.(iso3,year,measure,unit,age_group,sex,best,lo,hi,analysed)],
       file=fn)
...
tmpD <- dcast(db1f[pattern=='d',.(iso3,age_group,sex,best,lo,hi)],iso3~age_group+sex,
              value.var = c('best','lo','hi'))
tmpC <- dcast(db1f[pattern=='c',.(iso3,age_group,sex,best,lo,hi)],iso3~age_group+sex,
              value.var = c('best','lo','hi'))
tmpB <- dcast(db1f[pattern=='b',.(iso3,age_group,sex,best,lo,hi)],iso3~age_group+sex,
              value.var = c('best','lo','hi'))
tmpA <- merge(tmpB,tmpC)
tmpA <- merge(tmpA,tmpD)
tmpA <- merge(tmpA,est[year==estyr][,.(iso3,inc.num,inc.lo.num,inc.hi.num)])
...
## save
fn <- here('incsplits/data/A_country_incidence_disaggregated_age_sex_num_')
fn <- fn + getdate() + '.csv'
write.csv(PN,file=fn)
...
## save
fn <- here('incsplits/data/A_country_incidence_disaggregated_age_sex_rate_')
fn <- fn + getdate() + '.csv'
write.csv(PR,file=fn)
